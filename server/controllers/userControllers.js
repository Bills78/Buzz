...
module.exports.newuserPost = async (req, res) => {
	...
	try {
		if (takenName && takenEmail) {
			res
				.status(401)
				.json({ message: 'This account is alrady exists. Try logging in!' });
		} else if (takenName) {
			res
				.status(401)
				.json({
					message: 'This username is already in use. Try a another username!',
				});
		} else if (takenEmail) {
			res
				.status(401)
				.json({
					message:
						'This email is already in use. You may have an account already.',
				});
		} else {
